...
public interface IWorkspaceRepository
{
    ...
    Task<Workspace?> GetByIdAsync(long id, CancellationToken cancellationToken = default);
    void Add(Workspace workspace);
}

public interface IUserRepository
{
    Task<User?> GetByEmailAsync(long workspaceId, string email, CancellationToken cancellationToken = default);
    Task<User?> GetByIdAsync(long id, CancellationToken cancellationToken = default);
    Task<bool> EmailExistsAsync(long workspaceId, string email, CancellationToken cancellationToken = default);
    void Add(User user);
    ...
    Task<IReadOnlyList<long>> GetRoleIdsAsync(long userId, CancellationToken cancellationToken = default);

    /// <summary>Counts active users holding the given role (for last-owner protection).</summary>
    Task<int> CountActiveByRoleAsync(long roleId, CancellationToken cancellationToken = default);
}

/// <summary>A permission grant (permission + data scope) for a role.</summary>
public readonly record struct PermissionGrantData(long PermissionId, DataScope Scope);
...
public interface IRoleRepository
{
    ...
    Task<IReadOnlyList<long>> FilterExistingIdsAsync(IReadOnlyCollection<long> ids, CancellationToken cancellationToken = default);
    void Add(Role role);
    void Remove(Role role);

    /// <summary>Replaces a user's (workspace-wide) role assignments.</summary>
    Task SetUserRolesAsync(long workspaceId, long userId, IReadOnlyCollection<long> roleIds, CancellationToken cancellationToken = default);

    Task<bool> CodeExistsAsync(string code, CancellationToken cancellationToken = default);
    Task<int> CountAssignmentsAsync(long roleId, CancellationToken cancellationToken = default);

    Task<IReadOnlyList<PermissionGrantData>> GetRolePermissionsAsync(long roleId, CancellationToken cancellationToken = default);

    /// <summary>Replaces a role's permission grants (added via DbSet so EF tracks them as inserts).</summary>
    Task ReplaceRolePermissionsAsync(long workspaceId, long roleId, IReadOnlyCollection<PermissionGrantData> grants, CancellationToken cancellationToken = default);
}
...
public interface IPermissionRepository
{
    ...
    Task<IReadOnlyList<long>> FilterExistingIdsAsync(IReadOnlyCollection<long> ids, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<UserPermission>> ListUserOverridesAsync(long userId, CancellationToken cancellationToken = default);
    void AddUserOverride(UserPermission overrideEntry);
    Task RemoveUserOverridesAsync(long userId, CancellationToken cancellationToken = default);
}

public interface IRefreshTokenRepository
{
    Task<RefreshToken?> GetByHashAsync(string tokenHash, CancellationToken cancellationToken = default);
    Task RevokeAllForUserAsync(long userId, DateTimeOffset now, CancellationToken cancellationToken = default);
    void Add(RefreshToken token);

    /// <summary>Active (non-revoked, unexpired) sessions for a user.</summary>
    Task<IReadOnlyList<RefreshToken>> ListActiveAsync(long userId, DateTimeOffset now, CancellationToken cancellationToken = default);

    /// <summary>Revokes a single session by id, scoped to the owning user. Returns true if found.</summary>
    Task<bool> RevokeByIdAsync(long id, long userId, DateTimeOffset now, CancellationToken cancellationToken = default);
}
...
{
    Task<WorkspaceSecurityPolicy?> GetForWorkspaceAsync(long workspaceId, CancellationToken cancellationToken = default);
...
public interface IEmployeeRepository
{
    ...
    Task<IReadOnlyDictionary<long, Employee>> GetByUserIdsAsync(
